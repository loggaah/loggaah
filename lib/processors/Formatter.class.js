...
/**
 * A collection of patterns to look for in a message.
 */
const patterns = [];
    ...
    process(events, done) {
        for (let event of events) {
            ...
            for (let pattern of patterns) {
                let patternStart = message.indexOf(pattern.matcher);
                if (patternStart != -1) {
                    patternStart += pattern.matcher.length;
                    let parameter, token;
                    if (message.charAt(patternStart) == '{') {
                        let patternEnd = message.indexOf("}", patternStart);
                        parameter = message.substring(patternStart + 1, patternEnd);
                        token = pattern.matcher + "{" + parameter + "}";
                    } else {
...
Formatter.addPattern("level lvl p", (event, parameter) => {
    let length = event.getLevel().length;
    try {
