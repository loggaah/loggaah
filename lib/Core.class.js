...
/**
 * @property {Object} _lastConfiguration    The last configuration received by any of the configurators.
 */
class Core {
    /**
    ...
    configure(configuration) {
        if (!_.isEqual(configuration, this._lastConfiguration)) {
    ...
    configureConfigurator(type, configuration) {
        try {
            var configurator = Plugins.getInstance(type, 'configurator');
    ...
    process() {
        for (let event of this._events) {
            for (let pattern in this._loggers) {
                // TODO this can be very expensive, appenders could be attached to loggers during configuration.
                if (event.source.match(pattern)) {
                    if (this._loggers[pattern].appenders) {
    ...
    reset() {
        ...
        this._loggers = {};
        this._rules = {};
        delete this._lastConfiguration;
    }
