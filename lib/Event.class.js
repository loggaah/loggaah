...
class Event {
     ...
     * @param {Logger} logger
     */
    constructor(source, level, pattern, mdc, error, params, logger) {
    ...
    /**
     * Returns the logger that has produced this event.
     * @returns {Logger}
     */
    get logger() {
        return this._logger;
    ...
    /**
     * Returns a unique id that can be used to identify a log message later. If you want to see the hash id printed in
     * your logs you should make sure to include '%id' or '%hash' in your formatter configuration.
     * @returns {string} A referable id
     */
    getId() {
        if (!this._id) {
            this._id = makeHash();
    ...
    /**
     * Calling exit will terminate the program while trying to log any remaining log messages.
     */
    exit() {
        Core.exit();
    }
