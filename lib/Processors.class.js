...
class Processor extends Plugin {
     ...
     * @private
     */
    __proceed(events, processors, appender) {
        // TODO move this logic into the logger where all logic is combined
        this.process(events, (events) => {
            if (!processors.length) {
                require('./Configuration.class').appenders.__instances[appender].append(events);
                return;
            }
            var next = processors.shift();
            require('./Configuration.class').processors.__instances[next].__proceed(events, processors, appender);
        });
    }

    /**
     * Continues in the processor chain.
     * @param {Event[]} events      The event to be processed
